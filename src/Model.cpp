...
void Model::loadMeshes(const aiScene* pScene, bool FitSize)
{
    ...
    for(unsigned int i = 0; i < pScene->mNumMeshes; i++) {
        ...
        for(unsigned int j = 0; j < aim.mNumVertices; j++) {
            ...
            // Normale
            if(aim.HasNormals()) m.VB.addNormal(aim.mNormals[j].x, aim.mNormals[j].y, aim.mNormals[j].z);
            
            // Textur koordianaten
            if(aim.HasTextureCoords(0) && aim.mNumUVComponents[0] == 2) {
                m.VB.addTexcoord0(aim.mTextureCoords[0][j].x, aim.mTextureCoords[0][j].y);
            }
            
            // Vertex
            Vector v(aim.mVertices[j].x, aim.mVertices[j].y, aim.mVertices[j].z);
            m.VB.addVertex(v * factor);
        }
...
void Model::loadMaterials(const aiScene* pScene)
{
    ...
    for(unsigned int i = 0; i < pScene->mNumMaterials; i++) {
        ...
        if(aim.GetTextureCount(aiTextureType_DIFFUSE) == 1) {
            ...
            aiString name;
            aim.Get(AI_MATKEY_TEXTURE(aiTextureType_DIFFUSE, 0), name);
            std::string texPath = Path;
            texPath.append(name.C_Str());
